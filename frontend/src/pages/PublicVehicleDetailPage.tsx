...
export const PublicVehicleDetailPage: React.FC = () => {
  ...
  const seoTitle = `${vehicle.brand} ${vehicle.model} ${vehicle.version || ''} ${vehicle.year_model} - ${store.name}`;
  const seoDescription = `${vehicle.brand} ${vehicle.model} ${vehicle.version || ''}, ano ${vehicle.year_model}, ${vehicle.mileage.toLocaleString('pt-BR')} km, ${vehicle.fuel}, ${vehicle.transmission}. Preço: R$ ${vehicle.sale_price.toLocaleString('pt-BR')}. ${vehicle.description ? vehicle.description.substring(0, 150) : ''}`;
  const seoImage = photos[0]?.url || store.logo_url;
  const seoUrl = `${window.location.origin}/${slug}/vehicles/${vehicleId}`;
                  ...
                  <form onSubmit={handleProposalSubmit} className="space-y-4">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, proposal_value: e.target.value })
                        }
                        className="input input-bordered w-full"
                        placeholder="R$ 0,00"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, trade_vehicle: e.target.value })
                        }
                        className="input input-bordered w-full"
